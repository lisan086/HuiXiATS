    ...
    public class ZhiLingSaoMaQiLei : ABSNSheBei
    {
        ...
        {          
            ...
            if (isfanhi)
            {
                ...
                if (model.IsDu == CunType.XieModBusRTUDan1FanHui || model.IsDu == CunType.XieModBusRTUDan2FanHui || model.IsDu == CunType.XieModBusRTUDan4FanHui)
                {
                    int shuliang = model.IsDu == CunType.XieModBusRTUDan1FanHui ? 6 : model.IsDu == CunType.XieModBusRTUDan2FanHui ? 7 : 9;
                    SerialDataGuDingJieXi serialDataGuDingJieXi = new SerialDataGuDingJieXi();
                    for (; ZongKaiGuan;)
                    {
                        ...
                        serialDataGuDingJieXi.JieXiWanMeiData(shuliang,new byte[] { zhiling[0], zhiling[1] },false,null,false);
                        if (serialDataGuDingJieXi.DataCount>0)
                        {
                            PeiZhiLei.DataMoXing.SetJiCunQiValue(model,serialDataGuDingJieXi.GetShiShiData());
