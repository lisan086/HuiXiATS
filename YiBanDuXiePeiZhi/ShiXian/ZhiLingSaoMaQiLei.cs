...
namespace YiBanSaoMaQi.ShiXian
{
    public class ZhiLingSaoMaQiLei : ABSNSheBei
    {
        ...
        private void JieRu(List<JiCunQiModel> canshus)
        {
            foreach (var item in canshus)
            {
                ...
                if (ipcom != null)
                {
                    ...
                    {
                        SengData[ipcom.ZongSheBeiId].Add(new List<CunModel>() { ipcom.FuZhi() });
                    }
                }
            }
        }
            ...
            while (ZongKaiGuan)
            {
                ...
                if (dumodels.Count>0)
                {
                    for (int i = 0; i < dumodels.Count; i++)
                    {
                        ...
                        {
                            WriteRec(lianjieqi, dumodels[i],true);
                        }
                    }
                }
                #endregion
        ...
        public void WriteRec(ABSLianJieQi lianjieqi, CunModel model, bool isdu = false)
        {
            if (isdu == false)
            {
            ...
            if (model != null)
            {
                ...
                if (data != null)
                {
                    ZhenShiXie(model, lianjieqi, data, fanhu, isdu);
                    return;
                }
        ...
        private void ZhenShiXie(CunModel model, ABSLianJieQi lianjieqi, byte[] zhiling,bool isfanhi, bool isdu)
        {          
